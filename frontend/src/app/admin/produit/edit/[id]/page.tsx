...
export default function EditProductPage() {
    ...
    const loadData = async () => {
      try {
        ...
        setFormData({
          ...
          image: null,
          categoryIds: productData.categoryId ? [productData.categoryId.toString()] : []
        });
        
        // Définir les catégories sélectionnées
        if (productData.categoryId) {
          setSelectedCategories([productData.categoryId.toString()]);
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Mettre à jour la quantité en stock dans la base de données
      const quantityResponse = await fetch(`/api/admin/products/quantity/${productId}`, {
        method: 'PUT',
        headers: {
        ...
        body: JSON.stringify({ 
          ...
          operation: 'update' // 'update' pour produit existant, 'add' pour nouveau produit
        }),
      });

      let quantityResult = null;
      if (quantityResponse.ok) {
        quantityResult = await quantityResponse.json();
        console.log('✅ Quantity updated in database:', quantityResult);
      } else {
        console.log('⚠️ Quantity update failed, continuing with product update');
      }

      // 2. Mettre à jour les autres informations du produit
      const productData = new FormData();
      
      // Add all form fields except quantity (already updated)
      Object.keys(formData).forEach(key => {
        if (key !== 'image' && key !== 'categoryIds' && key !== 'stockQuantity') {
      ...
      // Add slug for database constraint
      ...
      if (response.ok) {
        ...
        let successMessage = 'Produit mis à jour avec succès';
        if (quantityResult && quantityResult.success) {
          successMessage += ` et quantité en stock mise à jour (${formData.stockQuantity} unités)`;
        }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center">
              ...
              <div className="ml-6">
                <h1 className="text-xl font-bold text-gray-900">Modifier Produit</h1>
                <p className="text-xs text-gray-500">Modifier {product?.name || 'le produit'}</p>
              </div>
            </div>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="mt-8 flex justify-end space-x-4">
              ...
              >
                ...
                <span>{isLoading ? 'Mise à jour...' : 'Mettre à jour le produit'}</span>
              </button>
            </div>
          </div>
        </form>
