...
export default function LoginPage() {
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Utiliser le service Faroty pour la connexion (envoi OTP)
      const response = await farotyService.sendOtpCode(email);
      
      if (response.success && response.data?.tempToken) {
        setTempToken(response.data.tempToken);
        setStep('otp');
        setSuccess("Code de vérification à 5 chiffres envoyé à votre email");
        setShowOtp(true);
      } else {
      ...
      <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-8">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-2xl font-bold text-gray-900 mb-2">
            {step === 'email' ? 'Connexion Faroty' : 'Vérification du code'}
          </h1>
          <p className="text-gray-600">
            {step === 'email' 
              ? 'Connectez-vous pour finaliser votre paiement' 
              : 'Entrez le code à 5 chiffres reçu par email pour vous connecter'
            }
          </p>
        </div>
        ...
        <div className="mt-8 pt-6 border-t border-gray-200">
          <div className="text-center text-sm text-gray-600">
            <p className="mb-2">🔐 Connexion sécurisée via Faroty</p>
